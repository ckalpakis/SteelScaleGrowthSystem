...
// ---------------------------------------------------------------------------
// Leads over time — area + line, with an optional "won" overlay and hover tips.
// ---------------------------------------------------------------------------
export function LeadsLineChart({ series }: { series: TimePoint[] }) {
  ...
  const leadsPath = series.map((p, i) => `${i === 0 ? "M" : "L"} ${x(i)} ${y(p.leads)}`).join(" ");
  const areaPath =
    n > 0
      ? `${series.map((p, i) => `${i === 0 ? "M" : "L"} ${x(i)} ${y(p.leads)}`).join(" ")} L ${x(n - 1)} ${padT + innerH} L ${x(0)} ${padT + innerH} Z`
      : "";
  const hasWon = series.some((p) => p.won > 0);
  const wonPath = series.map((p, i) => `${i === 0 ? "M" : "L"} ${x(i)} ${y(p.won)}`).join(" ");

  // Gridlines + y labels at 0, 1/2, max.
  const ticks = [0, Math.round(niceMax / 2), niceMax].filter((v, i, a) => a.indexOf(v) === i);
  // Show ~6 x labels evenly.
    ...
    <div ref={ref} className="relative w-full">
      {width > 0 && (
        <svg width={width} height={H} onMouseMove={onMove} onMouseLeave={() => setHover(null)} className="block">
          ...
          {areaPath && <path d={areaPath} fill="url(#leadsFill)" />}
          {leadsPath && <path d={leadsPath} fill="none" stroke={BLUE} strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />}
          {hasWon && wonPath && (
          ...
          {hover != null && (
            <g>
              ...
              <circle cx={x(hover)} cy={y(series[hover].leads)} r={4.5} fill={BLUE} stroke="#fff" strokeWidth={2} />
