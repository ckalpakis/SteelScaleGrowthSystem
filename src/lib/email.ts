...
export async function sendNewLeadEmail(client: Client, lead: Lead) {
  ...
  if (!client.email) {
    console.warn(`[email] client ${client.id} has no email — skipping notification`);
    return;
  }
  ...
  try {
    await resend.emails.send({
      ...
      html: renderLeadEmail(client, lead),
    });
  } catch (err) {
...
function renderLeadEmail(client: Client, lead: Lead) {
  const rows = [
    ["Name", lead.name],
    ["Phone", lead.phone],
    ["Email", lead.email],
  ...
  <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto">
    <h2 style="color:#111827">New lead for ${escapeHtml(client.business_name)}</h2>
